...
final sumupPaymentServiceProvider = Provider<PaymentService>((ref) {
  ...
  if (demoMode.enabled) {
    return DemoSumUpPaymentService(ref.watch(demoSandboxFactoryProvider));
  }
...
final nfcPaymentServiceProvider = Provider<PaymentService>((ref) {
  final demoMode = ref.watch(demoModeProvider);
  if (demoMode.enabled) {
    return DemoNfcPaymentService(ref.watch(demoSandboxFactoryProvider));
  }
...
final paypalPaymentServiceProvider = Provider<PaypalPaymentService>((ref) {
  final demoMode = ref.watch(demoModeProvider);
  if (demoMode.enabled) {
    return DemoPaypalPaymentService(ref.watch(demoSandboxFactoryProvider));
  }
...
final tseServiceProvider = Provider<TseService>((ref) {
  final demoMode = ref.watch(demoModeProvider);
  if (demoMode.enabled) {
    return DemoTseService(ref.watch(demoSandboxFactoryProvider));
  }
  return TseService();
});
...
class PaymentController extends FamilyNotifier<PaymentState, PaymentSession> {
    ...
    return PaymentState(
      ...
      paymentStatuses: <PaymentStatusSummary>[
        PaymentStatusSummary(
          label: 'SumUp',
          available: demoMode.enabled || ProductionConfig.hasSumUpConfig,
          message: demoMode.enabled
              ? 'SumUp Sandbox-Terminal aktiv.'
              : ProductionConfig.hasSumUpConfig
        ...
        PaymentStatusSummary(
          label: 'PayPal',
          available: demoMode.enabled || ProductionConfig.hasPaypalConfig,
          message: demoMode.enabled
              ? 'PayPal Sandbox aktiv.'
              : ProductionConfig.hasPaypalConfig
                  ? 'PayPal-Konfiguration vorhanden.'
                  : 'PayPal nicht konfiguriert.',
        ),
        PaymentStatusSummary(
          label: 'TSE',
          available: demoMode.enabled || ProductionConfig.hasFiskaltrustConfig,
          message: demoMode.enabled
              ? 'fiskaltrust Sandbox verbunden.'
              : ProductionConfig.hasFiskaltrustConfig
                  ? 'TSE konfiguriert.'
                  : 'fiskaltrust ist nicht konfiguriert.',
        ),
      ],
      trainingMode: demoMode.enabled || ProductionConfig.trainingModeDefault,
    );
  }
  ...
  Future<String> completePayment() async {
    ...
    final receipt = GuestReceiptData(
      ...
      exchangeRateInfo: BrandingConfig.current.texts.receiptExchangeRateInfo,
      orderDiscountLabel: BrandingConfig.current.texts.receiptNoDiscountLabel,
  ...
  Future<PaymentExecutionResult> _executeSelectedPayment({
    ...
    switch (state.method) {
      ...
      case PaymentMethod.paypal:
        return ref.read(paypalPaymentServiceProvider).executePayment(request);
